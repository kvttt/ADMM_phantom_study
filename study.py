...
def get_alpha_schedule(k, max_iter_admm, alpha_start=0.8, alpha_end=0.8):
...
if __name__ == "__main__":
    ...
    show_recon_plot = False
    show_rmse_plot = False
    show_rho_alpha_plot = True

    # rho and alpha schedule
    rho0 = 1.0  # initial rho
    gamma = 1.0  # rho scaling factor
    alpha_start = 1.0  # initial alpha
    alpha_end = 1.0  # final alpha
    ...
    if show_recon_plot:
        ...
        fig.savefig('phantom_recon.png', dpi=300)
        plt.show()

    # plot nrmse
    if show_rmse_plot:
        fig, axes = plt.subplots(1, 1, figsize=(9, 6), layout='constrained')
        axes.plot(history['rmse_all'], label='nRMSE after ADMM', marker='o')
        axes.set_xlabel('ADMM Iteration')
        axes.set_ylabel('nRMSE')
        axes.set_xticks(range(0, len(history['rmse_all'])))
        axes.set_title('nRMSE vs ADMM Iteration')
        axes.legend()
        axes.grid(True)
        fig.savefig('phantom_rmse.png', dpi=300)
        plt.show()
    ...
    if show_rho_alpha_plot:
        ...
        axes.grid(True)
        fig.savefig('phantom_rho_alpha.png', dpi=300)
        plt.show()
